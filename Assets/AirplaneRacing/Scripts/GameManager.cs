...
{
    [Tooltip("Game ends when an agent collects this much Points")]
    ...
    [Tooltip("The player Airplane")]
    public AirplaneAgent player;

    [Tooltip("The ML-Agent opponent Airplane")]
    public AirplaneAgent opponent;

    [Tooltip("The Checkpoint area")]
    ...
    private void EndGame()
    {
        ...
        if (player.PointsObtained >= opponent.PointsObtained )
        {
    ...
    private void Update()
    {
        if (State == GameState.Playing)
        {
            // Check to see if time has run out or either agent got the max Points amount
            if (TimeRemaining <= 0f ||
                player.PointsObtained >= maxPoints ||
                opponent.PointsObtained >= maxPoints)
            {
                EndGame();
            }

            // Update the timer and Points progress bars
            uiController.SetTimer(TimeRemaining);
            uiController.SetPlayerPoints(player.PointsObtained / maxPoints);
            uiController.SetOpponentPoints(opponent.PointsObtained / maxPoints);
        }
        ...
        else
        {
            ...
            uiController.SetPlayerPoints(0f);
            uiController.SetOpponentPoints(0f);
        }
